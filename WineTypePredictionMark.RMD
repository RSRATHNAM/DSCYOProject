...
To build the K Nearest Neighbor model, I am training the model using some tuning parameters. I am using cross validation with 10 resampling with 90% of the data. I then decided to train the model with k value of 1,3,4,5 and 7. This is to determine the ideal k value that has relatively good accuracy.
The plan is then to use the k value to create the final model. 
...
## Results
__________________________________________________________________________________
As you can see from the below output that achieved an accuracy of more than 70% which is way better than the 60% expectation depicted in the internet sources
```{r cm}
...
Through the K nearest neighbor model we were able to predict the output classification i.e the wine type 1, 2 or 3 with over 60% accuracy. We achieved an accuracy of 73.68421% which is way better than the model that you would see from the internet sources. We exceeded our goal.  This exercise also proves the usefulness of the K nearest neighbor when the target output is more than 2 classifiers (in this case its 3.) Although this dataset is very little in size, this simple model did a pretty decent job in classifying the output (3 values) from multiple features (13 inputs.)
